...
> Stumbled on some interesting quotes today:
...
<figure>
  ...
  <figcaption>
    <strong>Fig 3:</strong> A Timeline of Chaos: Confirmed U.S. interventions by decade and region.  
...
<figure>
  ...
  <figcaption>
    ...
    📎 <em>Sources: National Security Archive, CRS Report R42738, RAND, U.S. Army War College</em>  
    [2] <a href="https://nsarchive.gwu.edu/dnsa-collections">National Security Archive</a>,  
    [4] <a href="https://sgp.fas.org/crs/natsec/R42738.pdf">Congressional Research Service</a>
  </figcaption>
</figure>
...
<!-- Load Leaflet (already compatible with GitHub Pages / Jekyll) -->
...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    var map = L.map('us-covert-map').setView([14.5, -90.5], 5); // Centered on Central America

    // Bright, clean OpenStreetMap tile layer
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; <a href="https://www.openstreetmap.org/">OpenStreetMap</a>',
      maxZoom: 18
    }).addTo(map);

    // Conflict locations and data
    const conflicts = [
    {
      name: "Guatemala Civil War (1960–1996)",
      lat: 15.5,
      lon: -90.3,
      description: "Sparked by a 1954 CIA coup, Guatemala descended into a 36-year war. U.S.-backed regimes carried out genocide against indigenous Maya populations, leading to over 200,000 deaths and mass displacement."
    },
    {
      name: "El Salvador Civil War (1980–1992)",
      lat: 13.7,
      lon: -89.2,
      description: "The U.S. pumped over $4 billion into El Salvador's military and death squads to crush a leftist insurgency. Civilian massacres like El Mozote stained American foreign policy and created waves of refugees."
    },
    {
      name: "Nicaragua Contra War (1981–1990)",
      lat: 12.8,
      lon: -86.3,
      description: "After the Sandinistas ousted the U.S.-backed Somoza dictatorship, the Reagan admin funneled arms and cash to the Contras—a proxy force infamous for war crimes. Congress banned it, so the White House lied. See: Iran-Contra."
    },
    {
      name: "Honduras: U.S. Base Ops (1980s)",
      lat: 14.8,
      lon: -87.2,
      description: "Though not officially at war, Honduras became a giant launchpad for U.S. ops in Nicaragua and El Salvador. Military aid flooded in, human rights abuses spiked, and Honduras spiraled into lawlessness and gang rule."
    }
  ];
    ...
    conflicts.forEach(c => {
      L.circleMarker([c.lat, c.lon], {
        radius: 8,
        fillColor: "#d62728", // bright red
        color: "#333",
        weight: 1,
        opacity: 1,
        fillOpacity: 0.8
      }).addTo(map).bindPopup(`<strong>${c.name}</strong><br><span style="font-size: 0.9em;">${c.description}</span>`);
    });
  });
</script>
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    var diagram = flowchart.parse(`
      st=>start: 2003: U.S. Invasion of Iraq
      op1=>operation: CPA Order 1:\nDe-Baathification
      op2=>operation: CPA Order 2:\nDissolution of Army
      op3=>operation: Hundreds of Thousands Fired
      op4=>operation: Sunni Disenfranchisement
      op5=>operation: Insurgency Begins
      op6=>operation: Al-Qaeda in Iraq (AQI)
      op7=>operation: Civil War + ISIS Emerges
...
No, the CIA didn’t stage the Mandalay Bay shooting. They didn’t need to. They were too busy toppling governments, arming jihadists, and turning entire countries into failed states.

That migrant caravan your uncle screams about on Facebook? That’s not a plot by Soros. That’s blowback from U.S. foreign policy.

Let’s get real.

The CIA has destabilized regions from Tehran to Tripoli. They’ve overthrown elected leaders, propped up dictators, flooded conflict zones with weapons, and trained death squads like it was a retirement hobby.

This isn’t fringe paranoia. This is documented, declassified fact. You can read the CIA’s own memos if you’ve got the stomach.

You want the real conspiracy?

It's not a fake shooting. It’s a black-budget machine that’s been running coups since 1947. Truman created the CIA. By the ‘50s, it was already running foreign elections like a Vegas casino—rigged and well-funded.

Here’s how it works:
Step one—call them communists.
Step two—crash their economy.
Step three—stage a coup and install a puppet.
And if that fails? Blow the whole damn country up and walk away.

Iran. Guatemala. Chile. Nicaragua. Iraq. Libya. Syria.

Each one a spark that lit a region on fire. And guess who gets to deal with the refugees?

We do.

Guatemalans fleeing a civil war we started. Hondurans fleeing narco-states we helped arm. Syrians and Iraqis escaping the rubble of our airstrikes and regime change experiments.

And Republicans have the gall to call it an invasion?

You can’t destabilize half the planet and then clutch your pearls when the smoke drifts across your border.

It’s not a border crisis. It’s a blowback crisis.

ISIS? That wasn’t born in a vacuum. That came from Iraq—after we toppled Saddam, disbanded the army, and tossed 400,000 pissed-off guys with guns into the streets.

Al-Qaeda? That was the thank-you card from the Mujahideen after Operation Cyclone. We handed them missiles and jihadi textbooks in the ‘80s to fight the Soviets. Then they pointed those weapons at us.

You want fewer terrorists? Maybe stop creating the conditions that breed them.

And red states—don’t think you’re off the hook. You cheered every war, cut every foreign aid budget, and now you’re crying about asylum claims.

Texas is spending over a billion a year on border militarization. California? A fraction of that—investing in actual immigration services. One’s a panic show. The other’s a policy.

And here’s the kicker: immigrants in red states pay more in taxes than they cost in services. Deport them, and your economy crashes harder than crypto in a bear market.

But this isn’t about facts, is it?

It’s about fear. Spectacle. Blaming brown people for problems white guys in suits created.

So here’s the truth bomb:

The CIA didn’t need crisis actors. They had real ones—with real weapons and real body counts. They didn’t fake carnage. They funded it.

Still think the deep state is a theory?

It’s not a theory. It’s the résumé of an empire.

You want to stop the chaos?

Stop lighting the damn match.
-->